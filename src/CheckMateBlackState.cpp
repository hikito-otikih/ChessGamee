...
CheckMateBlackState::CheckMateBlackState(StateStack& stack, Context context)
	: State(stack, context)
	, mPausedText()
{
	mShowText = true;
	...
	mPausedText.setString("Checkmate!");
...
CheckMateBlackState::~CheckMateBlackState()
{
}

void CheckMateBlackState::draw()
{
...
bool CheckMateBlackState::update(sf::Time dt)
{
	mTextEffectTime += dt;
...
bool CheckMateBlackState::handleEvent(const sf::Event& event)
{
