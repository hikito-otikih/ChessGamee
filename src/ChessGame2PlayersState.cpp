...
ChessGame2PlayersState::ChessGame2PlayersState(StateStack& stack, Context context)
...
void ChessGame2PlayersState::addLegalMoves()
{
	for (int i = 0; i < 8; i ++)
		for (int j = 0; j < 8; j++)
		{
...
void ChessGame2PlayersState::drawMakedMoves(sf::RenderWindow& window)
{
...
sf::Texture ChessGame2PlayersState::getTexture(int type)
{
	if (type == 0)
	{
...
std::string ChessGame2PlayersState::newPosition(int i, int j)
{
	std :: string ans = "";
	ans += char(j + 'a');
	ans += char(7 - i + '1');
...
std::string ChessGame2PlayersState::localPosition(int i, int j)
{
	std::string ans = "";
	ans += char(i + 'a');
	ans += char(j + '1');
...
void ChessGame2PlayersState::newBoard()
{
	for (int i = 0; i < 8; i++)
	{
...
void ChessGame2PlayersState::draw()
{
	std::cerr << "break point 1\n";
	sf::RenderWindow& window = *getContext().window;
	std::cerr << "break point 2\n";
	window.setView(window.getDefaultView());
	std::cerr << "break point 3\n";
	window.draw(mBackgroundSprite);
	std::cerr << "break point 4\n";
	drawMakedMoves(window);
	std::cerr << "break point 5\n";
	window.draw(mGUIContainer);
	std::cerr << "break point 6\n";
	...
	std::cerr << "break point 10\n";
	if (selectedPiece != sf::Vector2i(8, 8))
	{
		mPieces[selectedPiece.x][selectedPiece.y]->draw(window);
	}
	std::cerr << "break point 11\n";
	sf::Text text;
	std::cerr << "break point 12\n";
	text.setPosition(100, 0);
	std::cerr << "break point 13\n";
	...
	std::cerr << "break point 14\n";
	text.setFont(mFont);
	std::cerr << "break point 15\n";
	text.setCharacterSize(60);
	std::cerr << "break point 16\n";
	window.draw(text);
	std::cerr << "break point 17\n";
	if (board.inCheck())
	{
		sf::Text inCheck;
...
bool ChessGame2PlayersState::update(sf::Time dt)
{
	std::cerr << "updating\n";
	mEffectTime += dt;
	...
	if (mPlayer.promotionStatus != '\0')
	{
		...
		{
			if (turn == 1) mPieces[i][j]->setTexture(getContext().textures->get(Textures::WHITEQUEEN));
			else mPieces[i][j]->setTexture(getContext().textures->get(Textures::BLACKQUEEN));
		}
		if (mPlayer.promotionStatus == 'n')
		{
			if (turn == 1) mPieces[i][j]->setTexture(getContext().textures->get(Textures::WHITEKNIGHT));
			else mPieces[i][j]->setTexture(getContext().textures->get(Textures::BLACKKNIGHT));
		}
		if (mPlayer.promotionStatus == 'b')
		{
			if (turn == 1) mPieces[i][j]->setTexture(getContext().textures->get(Textures::WHITEBISHOP));
			else mPieces[i][j]->setTexture(getContext().textures->get(Textures::BLACKBISHOP));
		}
		if (mPlayer.promotionStatus == 'r')
		{
			if (turn == 1) mPieces[i][j]->setTexture(getContext().textures->get(Textures::WHITEROOK));
			else mPieces[i][j]->setTexture(getContext().textures->get(Textures::BLACKROOK));
		}
...
void ChessGame2PlayersState::MAKEMOVE(int i, int j)
{
	resetCurOffset = 1;
	removeds.clear();
	Sremoveds.clear();
...
bool ChessGame2PlayersState::handleEvent(const sf::Event& event)
{
	std::cerr << "handling event\n";
	...
	if (event.type == sf::Event::KeyPressed && event.key.code == sf::Keyboard::Escape)
		...
		if (event.type == sf::Event::MouseButtonPressed)
		{
			if (event.mouseButton.button == sf::Mouse::Left)
			{
				...
				{
					for (int j = 0; j < 8; j++)
						if (mPieces[i][j] != nullptr && mPieces[i][j]->getType() == turn)
						{
